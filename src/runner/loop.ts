...
export class RavelRunner implements Runner {
  constructor(private readonly model: ModelAdapter, readonly version = "ravel-runner/0.1") {}
  ...
  async run(context: RunnerContext): Promise<RunnerResult> {
    ...
    while(modelCalls<context.toolContext.policy.limits.maxModelCalls){
      await context.toolContext.recorder.append({runId:context.runId,type:"model.request",payload:{turn:modelCalls+1,messageCount:messages.length}});
      let output;
      try{
        output=await this.model.respond({instructions:context.instructions,messages,tools:context.toolRegistry.definitions()});
      }catch(error){
        await context.toolContext.recorder.append({runId:context.runId,type:"model.response",payload:{turn:modelCalls+1,error:error instanceof Error?error.message:String(error)}});
      ...
      for(const call of output.toolCalls){
        if(steps>=context.toolContext.policy.limits.maxSteps) return {reason:"step_limit",steps};
        let result:ToolResult;
        try{
          result=await context.toolRegistry.execute(context.toolRegistry.parseRequest(call.callId,call.name,call.args),context.toolContext);
        }catch(error){
