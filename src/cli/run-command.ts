...
export async function preflightStudyRun(skillPath: string, fixtureDir: string, requireApiKey = true, studyConfigPath?: string): Promise<void> {
  ...
  if (study.sandbox.image !== DEFAULT_SANDBOX_IMAGE) throw new Error("Study sandbox image does not match the pinned Ravel image");
...
export function registerRunCommand(program: Command, deps: RunCommandDependencies = {}): void {
  program.command("run").argument("<skill>", "Path to a skill directory").description("Run one controlled Ravel experiment")
